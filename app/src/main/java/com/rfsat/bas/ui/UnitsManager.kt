 ...
 */
object UnitsManager {
    private const val PREFS = "bas_units"
    ...
    // --- Ballistics units (merged from VTB) ---
    ...
    fun displaySpeed(mps: Double): Double = if (isImperial()) mps * MPS_TO_MPH else mps
    fun displayOffset(meters: Double): Double = if (isImperial()) meters * M_TO_IN else meters * 100.0
