...
class RegistrationOverlayView @JvmOverloads constructor(
     ...
     * null while they have not said — which is the normal case, because the
     * control is off by default.
     *
     * A HINT AND NOTHING ELSE. It does not move, crop or rescale the picture;
     * it is passed to BlackMarkDetector, which tries a window around it and
     * uses the result only if it beats the ordinary search. Null means the
     * detector is called exactly as it always was.
     */
    ...
    /** The crosshair follows the finger rather than needing to be picked up
     *  first: there is only one of it, and hunting for a grab handle on a
     *  phone held at arm's length is its own small misery. */
    ...
    /** A full-width crosshair rather than a small cross: the eye judges "is
     *  this line through the middle of the rings" far better than "is this dot
     *  on the middle", because a line can be compared along its whole
     *  length. */
    private fun drawCentre(canvas: Canvas) {
        ...
        canvas.drawCircle(vx, vy, 14f * resources.displayMetrics.density, paint)
    }
