...
import com.rfsat.bas.databinding.ActivityImportBinding
...
class ImportActivity : BaseActivity() {
    ...
    private fun doDetect() {
        ...
        runCatching {
            val c = com.rfsat.bas.scoring.ShotCountCheck.check(rules, holes.size)
            c.message?.let { Logger.w("ImportActivity", it); notifyUser(it) }
        }

        if (holes.isEmpty()) {
            binding.hdrDistribution.visibility = View.GONE
            binding.histogram.distribution = com.rfsat.bas.scoring.ShotDistribution.EMPTY
            ...
            why.append(com.rfsat.bas.ui.Bullets.list(listOf(
